...
class OurNetwork(object):
    def __init__(self, pc, vocab_size, tagset_size):
        edim = 50
        wsize = 5
        indim = edim * wsize
        ...
        self.lookup = pc.add_lookup_parameters((vocab_size, edim))
...
def main(train_fname, dev_fname, test_fname, output_fname):
    ...
    word_indexer = Indexer(train_wordset.union([utils.START, utils.UNK, utils.END]))

    dev_data, _, _ = utils.read_labeled_data(dev_fname)

    m = dy.ParameterCollection()
    network = OurNetwork(m, word_indexer.size, tag_indexer.size)
    ...
    def predict(word_list):
        ...
        for i in xrange(2, len(padded) - 2):
            ...
            for word in window:
                if word_indexer.contains_key(word):
                    inputs.append(word_indexer.to_index(word))
                else:
                    inputs.append(word_indexer.to_index(utils.UNK))
        ...
        for epoch in xrange(num_epochs):
            ...
            for wordlist, taglist in train_data:
                ...
                for i in xrange(2, len(padded_wordlist) - 2):
                    ...
                    inputs = [word_indexer.to_index(word) for word in window]
            ...
            if verbose:
                ...
                print 'epoch: %d, evaluated sentences: %d, elapsed: %d' % (epoch + 1, num_sentences, elapsed)
                if report_acc:
    ...
    train(5, verbose=True, report_acc=False)
